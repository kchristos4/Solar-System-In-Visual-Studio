...
extern glm::mat4 getModelMatrix2();
extern glm::mat4 getModelMatrix3();
...
int main(void)
{
	...
	window = glfwCreateWindow(800, 800, u8"Ηλιακο Συστημα", NULL, NULL);
	if (window == NULL) {
	...
	unsigned char* dataTEST = stbi_load("sun.jpg", &width, &height, &nrChannels, 0);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, dataTEST);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	stbi_image_free(dataTEST);

	glBindTexture(GL_TEXTURE_2D, textureTEST[1]);
	dataTEST = stbi_load("planet.jpg", &width, &height, &nrChannels, 0);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, dataTEST);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	stbi_image_free(dataTEST);
	...
	dataTEST = stbi_load("meteor.jpg", &width, &height, &nrChannels, 0);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, dataTEST);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	stbi_image_free(dataTEST);
	...
	do {
		...
		glm::mat4 ModelMatrix2 = getModelMatrix2();
		glm::mat4 ModelMatrix3 = getModelMatrix3();
		...
		glBindTexture(GL_TEXTURE_2D, textureTEST[0]);//                                SUN
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureTEST[1]);//                                    PLANET
		glUniform1i(*textureTEST, 0);
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureTEST[2]);//                                     METEOR
		glUniform1i(*textureTEST, 0);
		...
		if (CheckKeyState() == true) {
			termination = 1;
		}
		else if (CheckKeyState() == false) {
			termination = 0;
		}

	} // Check if the ESC key was pressed or the window was closed
	while (glfwGetKey(window, GLFW_KEY_Q) != GLFW_PRESS || termination == 0
